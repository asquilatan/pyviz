    ...
    useEffect(() => {
        ...
        if (data.__type__ === undefined && typeof data === 'object' && !Array.isArray(data)) {
            Object.entries(data).forEach(([key, value]) => {
                ...
                let neighbors: any = value;
                if (neighbors && neighbors.__type__ === 'list') {
        ...
        // Case 2: Object Graph
            ...
            while (queue.length > 0) {
                ...
                let neighbors = curr.data?.neighbors ?? curr.data?.adj ?? curr.data?.edges;

                // Handle TrackedList neighbors
                if (neighbors && neighbors.__type__ === 'list') {
                    neighbors = neighbors.items;
                }
...
const GraphBarChart = ({ data, changedIndices, theme, accesses = [], arrayId }: { data: any[], changedIndices: Set<number>, theme: AppTheme, accesses?: { type: 'read' | 'write', arrayId: string, index: number }[], arrayId?: string }) => {
        ...
        <div className="flex flex-col items-start mt-2 border-t pt-2 w-full" style={{ borderColor: theme.colors.border }}>
            <div className="flex items-end gap-1 h-32 w-full px-2">
                {data.map((item, idx) => {
                    ...
                    // Check for accesses
                    const access = accesses.find(a => a.index === idx && (!a.arrayId || !arrayId || a.arrayId === arrayId));
                    const isRead = access?.type === 'read';
                    const isWrite = access?.type === 'write';

                    let bgColor = isChanged ? theme.colors.accent : theme.colors.primary;
                    let opacity = isChanged ? 1 : 0.7;
                    let border = isChanged ? `1px solid ${theme.colors.accent}` : 'none';
                    ...
                    return (
                        <div key={idx} className="flex-1 flex flex-col items-center justify-end group h-full">
                            ...
                            <div
                                className="w-full rounded-t-sm transition-all duration-300 group-hover:-translate-y-1"
                                style={{
                                    height: `${height}%`,
                                    backgroundColor: bgColor,
                                    opacity: opacity,
                                    border: border,
                                    boxShadow: (isRead || isWrite) ? `0 0 8px ${bgColor}66` : 'none'
                                }}
                                title={`Index ${idx}: ${item} ${isRead ? '(READ)' : ''} ${isWrite ? '(WRITE)' : ''}`}
                            />
...
const GraphArray = ({ data, label, isDeque = false, prevData, theme, accesses = [], arrayId }: { data: any[], label: string, isDeque?: boolean, prevData?: any[], theme: AppTheme, accesses?: { type: 'read' | 'write', arrayId: string, index: number }[], arrayId?: string }) => {
    ...
    return (
        <div className="flex flex-col items-start w-full">
            ...
            {showBarChart && isNumeric ? (
                <GraphBarChart data={data} changedIndices={changedIndices} theme={theme} accesses={accesses} arrayId={arrayId} />
            ) : (
                /* Standard Box View */
                <div className="flex items-center gap-0.5 flex-wrap">
                    ...
                    {data.map((item, idx) => {
                        ...
                        if (isRead) {
                            borderColor = theme.colors.success; // Green for read
                            bgColor = `${theme.colors.success}33`;
                            zIndex = 10;
                        }

                        if (isWrite) {
                            borderColor = theme.colors.error; // Red for write (or orange/accent)
                            bgColor = `${theme.colors.error}33`;
                            zIndex = 10;
                        }

                        if (isDeque) {
                            borderColor = isChanged ? theme.colors.accent : theme.colors.accent;
                        }

                        return (
                            <div key={idx} className="flex flex-col items-center group mb-2 relative" style={{ zIndex }}>
                                <div className={`min-w-[40px] h-10 px-2 flex items-center justify-center border rounded-sm text-sm font-mono transition-all duration-200 hover:-translate-y-1 hover:shadow-md ${(isRead || isWrite) ? 'scale-110' : ''}`}
                                    style={{
                                        borderColor,
                                        backgroundColor: bgColor,
                                        color: textColor,
                                        boxShadow: (isRead || isWrite) ? `0 0 10px ${borderColor}66` : 'none'
                                    }}>
                                    {String(item)}
                                </div>
                                <div className="text-[9px] mt-1 opacity-30 group-hover:opacity-100 font-mono" style={{ color: theme.colors.textSecondary }}>{idx}</div>

                                {/* Access Indicator Label */}
                                {(isRead || isWrite) && (
                                    <div className="absolute -top-3 left-1/2 transform -translate-x-1/2 text-[8px] font-bold uppercase px-1 rounded text-white"
...
const DataNode = ({ name, value, type, theme, accesses = [] }: { name?: string, value: any, type?: string, theme: AppTheme, accesses?: { type: 'read' | 'write', arrayId: string, index: number }[] }) => {
    if (value === null) {
        return (
    ...
    if (Array.isArray(value)) {
        ...
        if (isSimple) {
            // Filter accesses for this array if we have an ID
            // Note: Simple arrays might not have ID attached in value directly if they are just raw JSON arrays
            // But our TrackedList logic attaches ID to the variable wrapper, not the array itself usually?
            // Wait, executionService attaches ID to the variable wrapper.
            // But here 'value' is the raw array.
            // We need to check if we can match accesses.
            // The 'accesses' prop contains all accesses for the step.
            // We need to know the ID of THIS array.
            // The ID is passed in the variable wrapper in executionService.
            // But DataNode receives 'value' which is just the array content.
            // We need to pass the ID down to DataNode.
                ...
                <div className="pl-4 border-l ml-1 my-1" style={{ borderColor: theme.colors.border }}>
                    {value.map((item, idx) => (
                        <div key={idx} className="my-0.5">
                            <DataNode value={item} theme={theme} accesses={accesses} />
                        </div>
                    ))}
                </div>
                <span style={{ color: theme.colors.textSecondary }}>]</span>
    ...
    if (['tuple', 'set', 'deque', 'list'].includes(value.__type__)) {
        const typeCharMap: any = { tuple: ['(', ')'], set: ['{', '}'], deque: ['deque([', '])'], list: ['[', ']'] };
        const [startChar, endChar] = typeCharMap[value.__type__] || ['<', '>'];
        const items = value.items || [];

        // Special handling for simple lists to show inline
        if (value.__type__ === 'list') {
            const isSimple = items.every((v: any) => v === null || typeof v !== 'object' || (typeof v === 'object' && v.__type__ === undefined && !Array.isArray(v)));
            if (isSimple && items.length > 0) {
                return (
                    <div className="flex items-center gap-2 font-mono text-sm">
                        {name && <span style={{ color: theme.colors.text }}>{name}:</span>}
                        <span style={{ color: theme.colors.textSecondary }}>[</span>
                        <span style={{ color: theme.colors.success }}>
                            {items.map((v: any) => JSON.stringify(v)).join(', ')}
                        </span>
                        <span style={{ color: theme.colors.textSecondary }}>]</span>
                    </div>
                );
                ...
                <div className="pl-4 border-l ml-1 my-1" style={{ borderColor: theme.colors.border }}>
                    {Object.entries(data).map(([k, v]) => (
                        <div key={k} className="my-0.5">
                            <DataNode name={k} value={v} theme={theme} accesses={accesses} />
                        </div>
                    ))}
                </div>
                <span style={{ color: theme.colors.textSecondary }}>{'}'}</span>
                    ...
                    viewMode === 'tree' ? (
                        ...
                        <div className="space-y-8 p-4 overflow-auto h-full">
                            {sortedKeys.filter((key) => {
                                ...
                                if (val && val.__type__ === 'list' && val.items && val.items.length > 0) return true; // TrackedList
                                ...
                                sortedKeys.filter((key) => {
                                    ...
                                    // Handle TrackedList
                                    const isTrackedList = val && val.__type__ === 'list';
                                    const listData = isTrackedList ? val.items : (Array.isArray(val) ? val : []);
                                    const listId = isTrackedList ? val.id : undefined;
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <div className="overflow-x-auto min-h-[120px] flex items-center justify-start w-full">
                                                {(Array.isArray(val) || isTrackedList) ? (
                                                    <GraphArray
                                                        data={listData}
                                                        label={name}
                                                        prevData={Array.isArray(prevVal) ? prevVal : (prevVal && prevVal.__type__ === 'list' ? prevVal.items : undefined)}
                                                        theme={theme}
